...
public class FileSystemResourceManagerTest extends LocalStoreTest implements ICoreConstants {
	...
	public void testCreateFile() throws Throwable {
		...
		assertEquals("1.4", file.getStore().fetchInfo().getLastModified(), ((Resource) file).getResourceInfo(false, false).getLocalSyncInfo());
		try {
	...
	public void testWriteProject() throws Throwable {
		...
		final FileStore fileStore = project.getStore();
		...
		long lastModified = dotProject.getStore().fetchInfo().getLastModified();
