...
public class IPathVariableTest extends ResourceTest {
	...
	public void testResolvePathWithMacro() {
		...
		IPath expected = new Path("/tmp/backup/extra/bar")
				.setDevice(WINDOWS ? "C:" : null);
	...
	public void testEclipseHome() {
		...
		IPath expected = new Path(Platform.getInstallLocation().getURL()
				.getPath()).append("bar");
		IPath actual = manager.resolvePath(path);
		assertEquals("1.0", expected, actual);
	}
	...
	public void testWorkspaceLocation() {
		...
		IPath expected = project.getWorkspace().getRoot().getLocation().append(
				"bar");
		IPath actual = manager.resolvePath(path);
		assertEquals("1.0", expected, actual);
	}
	...
	/**
	 * Test IProject.getVariableRelativePathLocation(IPath)
	 */

	public void testGetVariableRelativePathLocation() {
		IPath path = project.getWorkspace().getRoot().getLocation().append(
				"bar");
		IPath actual;
		IPath expected;
		/* Does not work on the test machine because ECLIPSE_HOME and WORKSPACE is the same location
		actual = project.getVariableRelativePathLocation(path);
		...
		actual = project.getVariableRelativePathLocation(new Path(
				"/nonExistentPath/foo"));
	...
	public void testConvertToRelative() {
		...
		IPath actual = null;
		try {
			actual = manager.convertToRelative(file, false, "ONE");
		} catch (CoreException e) {
			fail("0.2", e);
		}
		IPath expected = file;
		...
		try {
			actual = manager.convertToRelative(file, false, "TWO");
		} catch (CoreException e) {
			fail("2.1", e);
		}
		...
		try {
			actual = manager.convertToRelative(file, true, "ONE");
		} catch (CoreException e) {
			fail("3.1", e);
		}
		...
		try {
			actual = manager.convertToRelative(file, true, "TWO");
		} catch (CoreException e) {
			fail("5.4", e);
		}
		expected = new Path("TWO/file.txt");
		...
		try {
			actual = manager.convertToRelative(file, false, null);
		} catch (CoreException e) {
			fail("7.1", e);
		}
		expected = new Path("TWO/file.txt");
		...
		try {
			actual = manager.convertToRelative(file, true, null);
		} catch (CoreException e) {
			fail("9.1", e);
		}
		expected = new Path("foo/other/file.txt");;
